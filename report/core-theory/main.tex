...
$dQ/dP$ để chỉ tỉ số khả năng (\emph{likelihood ratio}) (hoặc tổng quát hơn
...
\begin{defn}
  ...
  đại diện cho giả thuyết không. \begin{enumerate}[label=(\roman*)]
    ...
    \item Một kiểm định (test) $\phi$ là một biến ngẫu nhiên nhận giá trị trong
      ...
      $\phi$ đối với $\mathcal{P}$ là $\expect_P[\phi]$. Một kiểm định $\phi$
...
Có bốn công cụ SAVI chính: e-process, p-process, $(1 - \alpha)$-confidence
sequence và level-$\alpha$ sequential test; dễ dàng nhận thấy rằng hai công
cụ sau gắn liền với một mức $\alpha \in [0, 1]$ được xác định trước, trong khi
hai công cụ đầu thì không. Đây lần lượt là các phiên bản tuần tự của
e-variable, p-variable, $(1 - \alpha)$-confidence interval và level-$\alpha$
test. Chúng ta sẽ tập trung chủ yếu vào e-process vì chúng là đối tượng
...
giả định rằng độc giả đã quen thuộc với các thuật ngữ trong tiến trình ngẫu
nhiên, được tóm tắt dưới đây.

Chúng ta làm việc trong một không gian xác suất có lọc (filtered) cố định,
nghĩa là $\mathcal{F}$ đề cập đến một sự lọc (filtration) $(\mathcal{F}_t)_{t
...
Thông thường $\mathcal{F}$ được chọn là sự lọc tự nhiên của dữ liệu, đó là
$\mathcal{F}_t := \sigma(X^t)$, trong đó $X^t = (X_1, \dots, X_t)$, với
...
một hàm đo được của $X^t$. Tuy nhiên, đôi khi $\mathcal{F}$ là một sự lọc thô
hơn (coarser) (chúng ta loại bỏ thông tin) hoặc phong phú hơn (richer) (ví dụ
cho phép $\mathcal{F}_0 = \sigma(U_0)$ đối với một biến ngẫu nhiên phân phối
đều $U_0$ độc lập với tất cả các yếu tố ngẫu nhiên khác).
...
không. Một level-$\alpha$ sequential test cho $\mathcal{P}$ là một tiến trình
...
Khi đó, chúng ta có thể dễ dàng xác định một sequential test bởi thời điểm dừng
$\tau := \inf\{t \geq 1 : \phi_t = 1\}$ với quy ước $\inf \emptyset = \infty$.
Khi đó, một level-$\alpha$ test yêu cầu $\tau < \infty$ với xác suất tối đa
$\alpha$ dưới mỗi $P \in \mathcal{P}$. Điều này có thể được diễn đạt lại là:
\begin{equation}
...
\begin{thm}[Bất đẳng thức Ville]
  ...
  đây sẽ đúng: \begin{align}
    & P\biggl( \exists t \in \mathbb{N} : M_t \geq \frac{1}{\alpha} \biggr)
    \leq \alpha \text{ với mọi } P \in \mathcal{P} \text{ và } \alpha \in [0,
    1]; \\
    \iff & \sup_{P \in \mathcal{P}} P\biggl( \sup_{t \in \mathbb{N}} M_t \geq
    \frac{1}{\alpha} \biggr) \leq \alpha \text{ với mọi } \alpha \in [0, 1]; \\
    \iff & \sup_{P \in \mathcal{P}, \tau \geq 0} P\biggl( M_\tau \geq
    \frac{1}{\alpha} \biggr) \leq \alpha \text{ với mọi } \alpha \in [0, 1], \\
  \end{align} trong đó $\tau$ là bất kỳ thời điểm dừng $\mathcal{F}$ nào nhận
  các giá trị trong $[0, \infty]$.
\end{thm}
...
\begin{defn}
  \begin{enumerate}[label=(\roman*)]
    ...
    \item Với $\alpha \in [0, 1]$, một $(1 - \alpha)$-confidence sequence cho
...
\begin{defn}
  $(\hat\mu_t \pm \bar{B}_t)_{t \ge 1}$ được gọi là $(1 - \alpha)$-asymptotic
  confidence sequence cho $\mu$ nếu tồn tại một $(1 - \alpha)$-confidence
  sequence cho $\mu$ được cho bởi $(\hat\mu_t \pm \bar{B}_t^\ast)_{t \ge 1}$ và
  $\bar{B}_t^\ast / \bar{B}_t \stackrel{a.s}{\to} 1$.
\end{defn}

\input{hypothesis-testing}
