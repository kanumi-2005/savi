...
\begin{exmp}
  ...
  chặn trong $[-1, 1]$ thì \begin{equation}
    ...
    \log ( 10.4 / \alpha ) }{ n } }
  \end{equation} là một $(1 - \alpha)$-\gls{cs} cho $\mu$, tương tự
  như \begin{equation}
    \bigcap_{s \le n} \frac{ \sum_{i = 1}^n X_i }{ n } \pm 1.71 \sqrt{ \frac{
      \log\log(2n) + 0.72 \log ( 10.4 / \alpha ) }{ n } }.
  \end{equation}
  Ta thấy rằng, độ rộng của $(1 - \alpha)$-\gls{cs} lớn hơn so với
  ...
  $1$). Tuy nhiên, theo định nghĩa của \gls{cs}, tỉ lệ bao phủ sai
  tích lũy luôn được đảm bảo dưới $\alpha$ do tính đồng thời của nó. Điều này
  cũng cho thấy tính hợp lệ tại mọi thời điểm của \gls{cs} so với
...
\begin{exmp}
  ...
  \end{equation} là một $(1 - \alpha)$-\gls{cs} cho phân vị $1/2$
(trung vị).
\end{exmp}
...
\begin{exmp}
  ...
  - \alpha)$-\gls{cs} đồng thời cho tất cả các phân vị.
\end{exmp}
...
$(C_t)_{t \ge 1}$ là một \gls{cs} cho $\mu$ với mọi chiến lược đặt
cược.
...
(2024), Jun \& Orabona (2024), và Shekhar \& Ramdas (2024). Theo đó, \gls{cs}
được định nghĩa dựa trên một tích phân hỗn hợp như sau:
\begin{equation}
...
đến tính hợp lệ thống kê (validity) của \gls{cs}.
...
thiệp trung bình (ATE) và \gls{cs} (AsympCS) mang lại một
